...
export default function NavMenu({ mode }: { mode: "center" | "auth" }) {
  ...
  const [isYorioOpen, setIsYorioOpen] = useState(false);
  ...
  if (mode === "center") {
    return (
      <nav aria-label="메인 네비게이션" className="flex items-center gap-6 sm:gap-8">
        ...
        <div
          className="relative py-1"
          onMouseEnter={() => setIsYorioOpen(true)}
          onMouseLeave={() => setIsYorioOpen(false)}
        >
          <button
            type="button"
            className="flex items-center gap-2 px-3 py-1.5 rounded-md transition-all duration-200 hover:bg-gray-100"
          ...
          >
            ...
            <ChevronDown
              size={18}
              className={`transition-transform duration-200 ${
                isYorioOpen ? "rotate-180 text-[var(--chalk-yellow)]" : "text-gray-400"
              }`}
            />
          </button>
          ...
          {isYorioOpen && (
            <div
              className="absolute left-1/2 -translate-x-1/2 top-full pt-3 z-50 min-w-[220px]"
              style={{ animation: "slideInUp 0.2s ease forwards" }}
            >
              <div
                className="chalk-box-straight shadow-md p-3 flex flex-col gap-1.5"
              ...
              >
                ...
                >
                  <span className="w-[0.85rem] h-[0.85rem] rounded-full bg-[var(--chalk-yellow)] inline-block flex-shrink-0" />
                  <span className="text-[1.05rem] font-medium text-gray-600 hover:text-gray-900 whitespace-nowrap" style={{ fontFamily: "var(--font-body)" }}>삼각형의 외심과 내심</span>
                </Link>
              </div>
            </div>
          )}
